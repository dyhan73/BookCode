...
from stock_common import getDateByPerent
...
class Predictors:
    ...
    def createPredictor(self, name):
        if name == 'logistic':
            return PredictorLR('logistic')
        elif name == 'rf':
            return PredictorRF('rf')
        elif name == 'svm':
            return PredictorSVM('svm')
        else:
            return None
    ...
    def dump(self):
        for a_code in self.items: #list(self.items.keys()):
            for a_predictor in list(self.items[a_code].keys()):
