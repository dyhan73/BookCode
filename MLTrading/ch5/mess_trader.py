...
class MessTrader(BaseCollection):
    ...
    def simulate(self):
        ...
        for model in list(self.portfolio.items.keys()):
            for a_item in self.portfolio.items[model]:
                ...
                print("model=%s, code=%s , count_correct=%s, hit_ratio=%0.2f"
                      % (model, a_item.code, count_correct, a_item.hit_ratio))


    def dump(self):
        print(">>> Portfolio.dump <<<")
        for _, value in self.items:   #list(self.items.keys()):
            for a_item in value:        #self.items[key]:
                print("... model=%s, code=%s, row_index=%s, position=%s"
                      % (a_item.model, a_item.code, a_item.row_index, a_item.position))
